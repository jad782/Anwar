...
2. جهّز جاهزاً:
   ...
   - اسم الحزمة: `com.alanwar.quran.jad`.
...
11. اختر هدف **AnwarWidget** ▸ Signing & Capabilities ▸ نفس **Team** ▸ تأكّد bundle id صار `com.alanwar.quran.jad.AnwarWidget`.
...
3. أنشئ مجموعة متغيّرات **appstore**: `APP_STORE_APP_ID`، `BUNDLE_ID=com.alanwar.quran.jad`، `APPLE_TEAM_ID`.
4. شغّل Workflow **Al-Anwar iOS → TestFlight** ▸ يبني ويرفع تلقائياً (الودجت محفوظ في مشروع ios المرفوع، فـCodemagic يبنيه معه).

> ⚠️ في App Store Connect: أنشئ أيضاً معرّف الودجت `com.alanwar.quran.jad.AnwarWidget` (Identifiers) ومنتجات الدعم IAP (راجع IAP-PRODUCTS.md) قبل أول رفع.
